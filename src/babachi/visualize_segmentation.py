...
class BabachiVisualizer:
    ...
    def post_draw_settings(self, ax, chromosome, y_min=0.8, y_max=6):
        ax.set_xlim(0, self.chromosomes_wrapper.chromosomes[chromosome])
    ...
    def add_babachi_estimations(self, fig, ax, chromosome, BAD_segments, cosmic_est=None,
                                BAD_color='#0072B2CC', COSMIC_color='#D55E00',
                                BAD_lw=10, COSMIC_lw=4):
        bar_positions = []
        bar_widths = []
        bar_colors = []
        gap = 1 / 500 * self.chromosomes_wrapper.chromosomes[chromosome]
        ...
        for index, (pl_chr, start, end, BAD, *values) in BAD_segments.iterrows():
            ...
            BADs.append(BAD)
        if last_end != self.chromosomes_wrapper.chromosomes[chromosome] + 1:
        ...
        borders_for_bars = [1] + borders_to_draw + [self.chromosomes_wrapper.chromosomes[chromosome] + 1]
        ...
        all_borders = [1] + segmentation_borders + [self.chromosomes_wrapper.chromosomes[chromosome] + 1]
        for i in range(len(all_borders) - 1):
            if BADs[i]:
                ax.axhline(y=BADs[i],
                           xmin=all_borders[i] / self.chromosomes_wrapper.chromosomes[chromosome],
                           xmax=all_borders[i + 1] / self.chromosomes_wrapper.chromosomes[chromosome],
        ...
        if cosmic_est is not None and not cosmic_est.empty:
            ...
            for index, (chrom, startpos, endpos, BAD) in cosmic_est.iterrows():
                if startpos - last_end >= self.chromosomes_wrapper.chromosomes[chromosome] * vd * 2:
                    if last_end == 1:
                        cosmic_borders += [startpos - self.chromosomes_wrapper.chromosomes[chromosome] * vd]
                    else:
                        cosmic_borders += [last_end + self.chromosomes_wrapper.chromosomes[chromosome] * vd,
                                           startpos - self.chromosomes_wrapper.chromosomes[chromosome] * vd]
            ...
            all_cosmic_borders = [1] + cosmic_borders + [self.chromosomes_wrapper.chromosomes[chromosome] + 1]

            for i in range(len(all_cosmic_borders) - 1):
                if COSMIC_BADs[i]:
                    ax.axhline(y=COSMIC_BADs[i],
                               xmin=all_cosmic_borders[i] / self.chromosomes_wrapper.chromosomes[chromosome],
                               xmax=all_cosmic_borders[i + 1] / self.chromosomes_wrapper.chromosomes[chromosome],
        ...
        cax.set_xlim(1, self.chromosomes_wrapper.chromosomes[chromosome])
        cax.set_ylim(0, 1)
